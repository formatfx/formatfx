...
export interface TextParseResult { ranges: JsonRange[]; errors: TextParseError[] }
...
function walkElement(el: ObjNode, path: NodePath, out: JsonRange[]): void {
  const children = member(el, 'children');
  if (children?.kind === 'arr') {
    children.items.forEach((c, i) => {
      if (c.kind === 'obj') walkElement(c, [...path, i], out);
    });
  }
  const card = member(el, 'customCardProps');
  if (card?.kind === 'obj') {
    const formatter = member(card, 'formatter');
    if (formatter?.kind === 'obj') walkElement(formatter, [...path, CARD_SEGMENT], out);
  }
...
export function parseJsonWithMap(text: string): TextParseResult {
  ...
  if (ctx.i >= text.length) {
    ...
    return { ranges: [], errors: ctx.errors };
  }
  ...
  if (root.kind === 'obj') {
    ...
    const rootEl = rowFormatter?.kind === 'obj' ? rowFormatter
      ...
      : root;
    walkElement(rootEl as ObjNode, [], ranges);
