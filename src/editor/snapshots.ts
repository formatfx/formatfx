 ...
 *   · { kind: 'view' }              — the main (view) formatter document
 *   · { kind: 'column', field }     — one column formatter's tree
 *   · { kind: 'all' }               — the view + every registered column
 *                                     formatter + the view name
 * The scope KEY ('view' | 'col:<field>' | 'all') partitions the store so the
 * snapshot menu lists only the history of what's selected, with the
 * "everything" group alongside — placement decides the primary action.
 *
 ...
 * adding past the cap evicts the oldest of THAT scope only.
 */
...
export interface SnapshotPayload {
  /** view: the whole main document (kind, root, viewExtras, tile dims). */
  doc?: FormatterDocument;
  /** column: that formatter's element tree. */
  root?: SPElement;
  /** all: the main document + the registered column formatters + view name. */
  all?: { doc: FormatterDocument; columnRefs: Record<string, SPElement>; viewName: string };
}
...
export function defaultLabel(scope: SnapshotScope, viewName: string, docKind: string): string {
  switch (scope.kind) {
    case 'view': return `${viewName} — ${docKind === 'tile' ? 'tile' : docKind} view`;
    case 'column': return `${scope.field} column`;
    default: return `${viewName} + column formatters`;
  }
}
