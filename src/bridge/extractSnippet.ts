 ...
 * snippet: a commented, unminified, GET-only script the user pastes into
 * devtools ON their list page. It captures fields (+choices +live column
 * formatters), views (+view formatters) and up to 10 rows into a FormatFX
 * List Snapshot (docs/CONNECTIVITY.md §3.1) and copies it to the clipboard.
 *
 * Auditability is the product feature here: a maker — or their IT — can
 * read every line before running it. No library blob, no write verbs.
...
export function buildExtractSnippet(opts: ExtractSnippetOptions = {}): string {
  ...
  return `/* FormatFX — live list extract (READ-ONLY: every request is a GET).
 *
